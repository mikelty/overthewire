...
decode_caesar = lambda c, shift: chr(ord('A') + (ord(c) - ord('A') - shift) % 26)


def decode_with_keylen(ciphers, target, keylen):
	ciphers=[cipher.replace(' ','') for cipher in ciphers]
	target=target.replace(' ','')
	freq_tables=[dict() for _ in range(keylen)]
	for freq_table in freq_tables:
		for c in string.ascii_uppercase:
			freq_table[c]=0
	for cipher in ciphers:
		for i,c in enumerate(cipher):
			freq_tables[i % keylen][c]+=1
	cipher_freqs=[''.join(e[0] for e in sorted(list(freq_table.items()),key=lambda x:-x[1])) \
	              for freq_table in freq_tables]
	shifts=[ord(cipher_freq[0])-ord('E') for cipher_freq in cipher_freqs]
	return ''.join(decode_caesar(c, shifts[i % keylen]) for i, c in enumerate(target))
...
if __name__ == '__main__':
	print(decode_with_keylen([found1, found2],krypton5,6))
